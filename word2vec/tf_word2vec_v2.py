...
def generate_batch(data, batch_size, num_skips, skip_window):
    ...
    print(batch.shape," ",context.shape)
...
with graph.as_default():
  ...
  weights = tf.Variable(
      tf.random.truncated_normal([embedding_size, vocabulary_size],
                          stddev=1.0 / math.sqrt(embedding_size)))
  biases = tf.Variable(tf.zeros([vocabulary_size]))
  hidden_out = tf.transpose(a=tf.matmul(tf.transpose(a=weights), tf.transpose(a=embed))) + biases

  # convert train_context to a one-hot format
  train_one_hot = tf.one_hot(train_context, vocabulary_size)

  cross_entropy = tf.reduce_mean(input_tensor=tf.nn.softmax_cross_entropy_with_logits(logits=hidden_out, labels=tf.stop_gradient(train_one_hot)))
