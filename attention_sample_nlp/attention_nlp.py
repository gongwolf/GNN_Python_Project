...
# %%
file_path = './mar-eng/mar.txt'  # please set the path according to your system
...
def load_dataset(pairs, num_examples):
    ...
    # English sentences by idx
    input_tensor = [[inp_lang.word2idx[s]
                     for s in en.split(' ')] for en, ma in pairs]

    # Marathi sentences by idx
    target_tensor = [[targ_lang.word2idx[s]
                      for s in ma.split(' ')] for en, ma in pairs]
...
input_tensor, target_tensor, inp_lang, targ_lang, max_length_inp, max_length_targ = load_dataset(
    sent_pairs, len(lines))

max_length_inp, input_tensor.shape
...
def predict_random_val_sentence():
    ...
    result, sentence, attention_plot = evaluate(
        random_input, encoder, decoder, inp_lang, targ_lang, max_length_inp, max_length_targ)
    ...
    attention_plot = attention_plot[:len(
        result.split(' '))-2, 1:len(sentence.split(' '))-1]
    sentence, result = sentence.split(' '), result.split(' ')
    sentence = sentence[1:-1]
    result = result[:-2]

    # use plotly to generate the heat map
    trace = go.Heatmap(z=attention_plot, x=sentence,
                       y=result, colorscale='Reds')
    data = [trace]
    iplot(data)
