...
def preprocess_sentence(w):
    ...
    w = re.sub(r"([?.!,¿])", r" \1 ", w)
    # instead of multiple space with only one space
    w = re.sub(r'[" "]+', " ", w)
...
def tokenize(lang):
    # keep all of the character of the input
...
class BahdanauAttention(tf.keras.Model):
    ...
    def call(self, query, values):
        ...
        # tf.nn.tanh(self.W1(values) + self.W2(hidden_with_time_axis)) shape is (batch_size, max_length, hidden size)
        # each row of a is added with b
        # score shape == (batch_size, max_length, 1)
...
print('Decoder output state shape: (batch_size, vocab size) {}'.format(
    sample_decoder_state.shape))
print('Decoder output attention weights shape: (batch_size, vocab size) {}'.format(
